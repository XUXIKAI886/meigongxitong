  ...
  async process(job: any): Promise<{ imageUrl: string; width: number; height: number }> {
    ...
    const response = await withRetry(async () => {
      return await this.imageClient.generateImage({
        prompt: `${enhancedPrompt}, storefront design, shop sign, commercial design, high quality, professional`,
    ...
    const targetSize = parseSize(config.images.sizes.storefront);
    const minSize = config.images.minSizes.storefront;
    
    // Ensure minimum size requirements
    if (targetSize.width < minSize.width || targetSize.height < minSize.height) {
      throw new Error(`Storefront size must be at least ${minSize.width}x${minSize.height}`);
    }
...
jobRunner.registerProcessor('generate-storefront', new StorefrontGenerationProcessor());
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error('Storefront generation API error:', error);
