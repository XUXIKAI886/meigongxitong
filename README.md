...
一个基于 Next.js 15 和 AI 技术的专业设计工具，为外卖商家提供一站式视觉营销解决方案。通过先进的人工智能算法，实现从单品图优化、品牌设计、食物替换到多图融合的全流程自动化处理，涵盖7大核心功能模块。

## 📊 项目状态

**🎯 开发完成度：100%**
- ✅ **7大核心功能全部实现**：所有功能模块已完成开发并稳定运行
- ✅ **API接口完整**：所有API路由全部实现，支持完整的业务流程
- ✅ **前端界面完善**：7个功能页面 + 首页，响应式设计，用户体验优秀
...
### 🗑️ 图片去食物工具
- **智能食物移除**：AI自动识别并移除图片中的所有食物元素
- **器皿完美保留**：完整保留碗、盘子、杯子等容器的样式、颜色和光泽
- **背景保持**：保持原图的背景、桌面、桌布、装饰品等非食物元素
- **文字100%保留**：完整保留图片中的所有文字内容（店名、品牌名、价格、标签、Logo、标语等）
- **清洁效果**：器皿内部干净、空的，没有任何食物残留
- **原图尺寸输出**：输出图片与上传图片尺寸完全一致
- **批量处理**：支持一次上传多张图片批量处理
- **高质量输出**：PNG格式，100%质量，使用Lanczos3算法
- **3次重试机制**：确保处理成功率
- **网格展示**：处理结果采用2列网格布局，方便查看和对比
- **适用场景**：头像、店招、海报等需要保留空器皿的设计素材制作
