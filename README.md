...
一个基于 Next.js 15 和 AI 技术的专业设计工具，为外卖商家提供一站式视觉营销解决方案。通过先进的人工智能算法，实现从单品图优化、品牌设计、食物替换到多图融合的全流程自动化处理，涵盖6大核心功能模块。
...
- **48个精选模板库**：头像16个、店招16个、海报16个，全部JPG格式优化
...
- **🆕 双平台下载功能**：
  - 🟡 **美团平台**：原图尺寸下载（头像800×800、店招1280×720、海报1440×480）
  - 🔵 **饿了么平台**：自动调整至平台规格下载（头像800×800、店招750×423、海报2048×600）
  - **智能尺寸转换**：Canvas API客户端实时调整，保持图片质量
  - **批量下载支持**：支持一键下载美团或饿了么全套三件套
  - **品牌配色设计**：美团黄色主题、饿了么蓝色主题，直观易识别
...
- **商业品质**：生成专业级的套餐营销图片
- **文件名保持**：下载的套餐图使用第一张源图片的文件名
- **输出规格**：1200×900px

### 🖥️ Tauri 桌面应用支持
- **跨平台桌面应用**：基于 Tauri 2.x 框架，支持 Windows、macOS、Linux
- **原生文件对话框**：使用系统原生保存对话框，提供更好的用户体验
- **高性能文件操作**：使用 Rust 后端处理文件 I/O，性能优异
- **完美兼容**：所有功能在桌面应用和 Web 浏览器中均可正常使用
- **图片下载优化**：
  - ✅ Web 环境：使用传统的 `<a download>` 标签下载
  - ✅ Tauri 环境：使用原生文件系统 API，支持自定义保存路径
  - ✅ 智能环境检测：自动识别运行环境，选择最优下载方式
  - ✅ 3参数 API 格式：使用 Tauri 2.x 官方验证的 `plugin:fs|write_file` 调用格式
- **技术实现**：
  - Dialog API: `invoke(cmd, { options: {...} })` - 2参数格式
  - FS Write API: `invoke(cmd, data, { headers: {...} })` - 3参数格式
  - 路径编码: `encodeURIComponent()` 处理 Windows 路径
  - 数据类型: `Uint8Array` 二进制数据传输
...
### v2.4.1 - 2025年10月 🎨 **双平台下载功能版**
- ✅ **双平台下载系统**：Logo设计工作室新增美团和饿了么双平台下载功能
  - 🟡 美团平台：原图尺寸下载（头像800×800、店招1280×720、海报1440×480）
  - 🔵 饿了么平台：自动调整至平台规格（头像800×800、店招750×423、海报2048×600）
- ✅ **智能尺寸转换**：基于Canvas API实现客户端实时图片尺寸调整
  - `resizeImage()` 函数：支持精确的宽高调整，保持图片质量
  - `downloadResizedImage()` 函数：集成调整和下载功能
  - 跨域图片处理：使用 `crossOrigin = 'anonymous'` 确保兼容性
- ✅ **批量下载支持**：
  - 美团三件套批量下载：一键下载原图尺寸全套设计
  - 饿了么三件套批量下载：一键下载平台规格全套设计
  - 文件名智能标注：自动添加平台标识和尺寸信息
- ✅ **品牌配色设计**：
  - 美团按钮：黄色主题（`border-yellow-500 text-yellow-700 hover:bg-yellow-50`）
  - 饿了么按钮：蓝色主题（`border-blue-500 text-blue-700 hover:bg-blue-50`）
  - 渐变背景批量下载按钮，提升视觉体验
  - SVG图层图标，增强按钮识别度
- ✅ **全面覆盖**：
  - 头像结果卡片：双按钮下载（美团版 + 饿了么版）
  - 店招结果卡片：双按钮下载（美团版 + 饿了么版）
  - 海报结果卡片：双按钮下载（美团版 + 饿了么版）
  - 批量下载区域：双按钮批量下载（美团三件套 + 饿了么三件套）
- 📝 **模板扩展**：新增模板19和20
  - 模板19：米煲正煲仔饭（头像/店招/海报）
  - 模板20：百草鲜卤铺（头像/店招/海报）
  - 自动店铺名填充功能
- 🔧 **技术实现**：
  - 影响文件：`src/lib/image-download.ts` - 新增尺寸调整函数
  - 影响文件：`src/app/logo-studio/page.tsx` - 双平台下载UI集成
  - 影响文件：`饿了么尺寸.md` - 平台规格文档
...
- ✅ **文件名保持功能**：为背景融合和多图融合工具添加原始文件名保持功能
  ...
  - 多图融合工具：使用第一张源图片的文件名作为套餐图文件名
  - 使用useRef避免React闭包问题，确保异步操作中文件名正确传递
  - 统一实现模式：与食物替换工具保持一致的文件名管理机制
- ✅ **Logo Studio修复**：修复Logo设计工作室Markdown图片链接支持问题
  - 新API客户端（ProductRefineApiClient）添加Markdown URL下载转换
  - 修复店招/海报/头像生成失败"No valid image data found"错误
  - 支持多种图片格式：Markdown链接、plain URL、base64、data URL
  - 与旧API客户端保持功能一致性
- ✅ **图片墙文案优化**：更新文案"系统将为您设计"改为"系统已为您设计"
- ✅ **用户体验提升**：下载文件名与上传文件名一致，便于商家管理和识别
...
4. **AI融合处理（两阶段流程）**：
   ...
   - **步骤2 - 店名替换**（Doubao API）：
     ...
     - 保持图片整体设计风格和视觉一致性
   - **技术要点**：
     - 步骤2必须上传完整的FormData（dishImage、avatarTemplate、avatarTemplateId）以通过后端验证
     - 实际处理使用step1ResultUrl作为图像源，但其他字段满足API契约完整性要求
     - 两阶段处理确保了菜品融合和店名个性化的最佳效果
5. **批量生成结果**：
   - 店招设计：1280×720px（适合外卖平台展示）
   - 海报设计：1440×480px（适合广告宣传推广）
   - 头像设计：800×800px（适合社交媒体标识）- 支持两阶段智能处理
6. **🆕 双平台下载选择**：
   - **单独下载** - 每张结果图提供双按钮：
     - 🟡 **美团版**：下载原图尺寸（头像800×800、店招1280×720、海报1440×480）
     - 🔵 **饿了么版**：自动调整至平台规格（头像800×800、店招750×423、海报2048×600）
   - **批量下载** - 一键下载全套三件套：
     - 🟡 **美团三件套**：批量下载原图尺寸全套设计
     - 🔵 **饿了么三件套**：批量下载平台规格全套设计
   - **智能尺寸转换**：基于Canvas API实时调整，无需后端处理，保持图片质量
   - **文件名智能标注**：自动添加平台标识和尺寸信息（如：`饿了么-川乐汇-店招-750x423.png`）
...
**美工设计系统 v2.4.1** 是一个完整的、生产就绪的 AI 图像设计平台，专为外卖行业打造：
...
### v2.3.1 文件名保持与修复升级
- **📁 文件名保持**：背景融合和多图融合工具实现原始文件名保持功能
- **🔧 Logo Studio修复**：修复Markdown图片链接支持，解决生成失败问题
- **🎯 用户体验提升**：下载文件名与上传一致，便于商家管理
- **🔄 统一实现模式**：与食物替换工具保持一致的文件名管理机制

### v2.3.0 Gemini API迁移升级
- **🔄 API端点迁移**：所有Gemini API调用迁移到新的OpenAI兼容端点
- **🎯 统一配置**：新增GEMINI_*统一环境变量，简化配置管理
- **📦 智能响应处理**：支持Markdown图片链接自动下载转换
- **✨ 多格式兼容**：同时支持base64、data URL、HTTP URL三种图片格式
...
### v2.4.0 (2025-10-17)
**🎨 AI提示词量化优化 - 全面提升视觉效果质量**

#### 🎯 核心优化：量化色彩增强提示词

**优化目标**：将描述性色彩增强语言改为精确的量化指令，显著提升AI生成图片的色彩鲜艳度和视觉吸引力。

**优化策略**：
- ❌ **原有方式**：使用模糊描述（如"大幅提升菜品颜色的鲜艳度和饱和度"）
- ✅ **优化后**：使用精确数值指令（如"青椒绿色→鲜嫩翠绿色(饱和度+50%,亮度+30%)"）

#### 📊 具体优化内容

##### 1️⃣ Logo设计工作室 (头像/店招/海报)
**影响文件**: `src/app/api/logo-studio/generate/route.ts`

**优化点**：
- ✨ **量化色彩增强**：添加7类食材的精确色彩调整指标
  - 青椒/青菜：饱和度+50%，亮度+30%
  - 肉类：饱和度+40%，亮度+25%
  - 辣椒/番茄：饱和度+60%，亮度+20%
  - 葱段/香菜：饱和度+50%，亮度+30%
  - 米饭/面条：亮度+40%
  - 酱汁/汤汁：饱和度+45%，亮度+25%
- ✨ **整体亮度提升**：30-40%整体亮度增强，消除暗淡灰暗效果
- ✨ **专业摄影效果**：高光效果、对比度增强、专业光线处理
- ✨ **份量控制优化**：添加7-8分满的食物份量要求，避免边缘留白过多
- ✨ **显示逻辑修复**：步骤1生成时清除步骤2结果，确保只显示最新内容

**修改位置**：
- 行731-776: `createFoodReplacementPrompt()` (头像步骤1)
- 行792-847: `createSimpleFusionPrompt()` (店招/海报)
- 行406, 440, 550: 显示状态清理逻辑 (page.tsx)
...
**优化点**：
- ✨ 与Logo Studio使用相同的量化色彩增强策略
- ✨ 添加6类酱汁/汤汁的专项色彩调整
- ✨ 7-8分满的份量控制要求
- ✨ 高光效果和专业摄影级光影处理

**修改位置**：
- `route.ts` 行192-269: 单图提示词优化
- `batch/route.ts` 行179-257: 批量提示词优化
...
- ✨ 7类食材的量化色彩增强（与其他模块一致）
- ✨ 专业摄影效果强化：
  - 高光与光泽效果（油亮效果、自然光泽、反光处理）
  - 质感与细节优化（蔬菜、肉类、米饭、炸物的具体质感）
  - 光影三维效果（戏剧性光照、明暗对比、影棚效果）
  - 商业级精修（去瑕疵、提升高级感、广告级后期）
- ✨ 统一为中文提示词，提升AI理解精度
- ⚠️ **注意**：背景融合不需要份量控制（整体融合场景，非替换到碗里）

**修改位置**：
- `route.ts` 行131-201: 单图提示词优化
- `batch/route.ts` 行223-293: 批量提示词优化
...
**一致性保证**：
现在所有涉及食物图片生成的功能都使用统一的色彩增强策略：
- ✅ Logo Studio (头像设计 - 步骤1)
- ✅ Logo Studio (店招/海报)
- ✅ 食物替换工具 (单图)
- ✅ 食物替换工具 (批量)
- ✅ 背景融合工具 (单图)
...
- `3a9edcc` - 优化Logo设计工作室食物替换效果和显示逻辑
- `e07cb73` - 统一食物替换工具提示词，与Logo Studio保持一致
- `e82e789` - 优化背景融合工具提示词，添加量化色彩增强和专业摄影效果

#### 🔧 技术细节

**提示词工程改进**：
- 从定性描述改为定量指令，提升AI执行精度
- 针对不同食材类型添加专项调整参数
- 增加具体的百分比数值（饱和度+40-60%，亮度+20-40%）
- 添加专业摄影术语和效果描述

**代码质量**：
- 保持文件规模控制要求（每个文件≤250行）
- 统一提示词结构和格式
- 保持向后兼容性
...
<div align="center">
  <p>🎨 <strong>美工设计系统 v2.4.0</strong> - 让 AI 为您的外卖生意增光添彩</p>
  <p>🚀 <strong>量化色彩增强 + 专业摄影效果</strong> | Made with ❤️ by AI Design Team | 2025年10月最新版</p>
  <p>🎯 <strong>视觉质量飞跃</strong> - 量化提示词工程，食物色彩更鲜艳，份量更饱满，光影更专业</p>
  <p>✨ <strong>全模块统一</strong> - Logo Studio、食物替换、背景融合全部使用相同的量化增强策略</p>
  <p>📊 <strong>数值化控制</strong> - 饱和度+40-60%，亮度+20-40%，专业摄影级效果</p>
</div>
